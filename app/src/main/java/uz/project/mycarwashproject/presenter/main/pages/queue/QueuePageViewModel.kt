...
class QueuePageViewModel(private var mainRepository: MainRepository) : ViewModel(),
    ...
    fun submitProposal(proposalModel: ProposalModel) {
        if (!isConnected()) {
            errorInternetLD.value = "Интернет не подключен!"
            return
        }
        successLD.value = mainRepository.submitProposal(proposalModel)
    }
